...
const TRANSLATE_CASES = [
  ...
  },
  {
    ...
    duration: 60,
    modelRaw: '-i input.mp4 -c:v libx264 clip-out.mp4',
    expectedArgs: ['-i', '/v/clip.mp4', '-ss', '10', '-frames:v', '1', 'clip-out.png'],
  },
  {
    name: 'last-keep fixes wrong lone -ss',
    instruction: 'keep the last 5 seconds',
    duration: 27.49,
    modelRaw: '-i input.mp4 -ss 10 clip-out.mp4',
    expectedArgs: ['-i', '/v/clip.mp4', '-ss', '22.49', 'clip-out.mp4'],
  },
  {
    name: 'gif with libx264 reverts to default encoder',
    instruction: 'Convert to gif',
    duration: 30,
    modelRaw: '-i input.mp4 -c:v libx264 -vf fps=10,scale=480:-1:flags=lanczos -an clip-out.gif',
    expectedArgs: ['-i', '/v/clip.mp4', '-vf', 'fps=10,scale=480:-1:flags=lanczos', '-an', 'clip-out.gif'],
  },
  {
    name: 'thumbnail with libx264 uses image encoder',
    instruction: 'Thumbnail at 10 seconds',
    duration: 60,
    modelRaw: '-i input.mp4 -c:v libx264 clip-out.mp4',
    expectedArgs: ['-i', '/v/clip.mp4', '-ss', '10', '-frames:v', '1', 'clip-out.png'],
  ...
  {
    name: 'slow motion under size limit budgets the longer output',
    instruction: 'slow motion, below 100MB',
    duration: 60,
    modelRaw: '-i input.mp4 -c:v libx264 -vf setpts=2*PTS -af atempo=0.5 -c:a aac clip-out.mp4',
    expectedArgs: ['-i', '/v/clip.mp4', '-c:v', 'libx264', '-vf', 'setpts=2*PTS', '-af', 'atempo=0.5', '-c:a', 'aac',
